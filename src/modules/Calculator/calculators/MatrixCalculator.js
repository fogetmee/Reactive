import { Matrix, Complex } from '../entities';
...
export default class MatrixCalculator  {
  ...
  sub(a, b) {
    return new Matrix(
      a.values.map((arr, i) => arr.map((elem, j) => elem - b.values[i][j]))
    );
  }
      ...
      for (let j = 0; j < a.values[i].length; j++) {
        ...
        for (let k = 0; k < a.values[i].length; k++) {
          if (a.values[i][k].im != 0 && b.values[k][j].im != 0) {
            ...
            s =
              s +
              a.values[i][k].re * b.values[k][j].re -
              a.values[i][k].im * b.values[k][j].im;
          }
          if (a.values[i][k].im == 0 && b.values[k][j].im != 0) {
  ...
  prod(p, a) {
    return new Matrix(
      a.values.map((arr) => arr.map((elem) => this.calc.mult(elem, p)))
    );
  }
